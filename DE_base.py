...
@jit(nopython=True)
def objective(indv, vt, voltage, current, points):
...
@jit(nopython=True)
def differential_evolution(popsize, gmax, vt, voltage, current, points):
    ...
    for i in range(popsize):
        score[i] = objective(pop[i, :], vt, voltage, current, points)
    ...
    for generation in range(gmax):
        for i in range(popsize):
            ...
            # --- Greedy Selection --- #
            if objective(trial[i, :], vt, voltage, current, points) < objective(pop[i, :], vt, voltage, current, points):
                pop[i, :] = trial[i, :]
            else:
                pop[i, :] = pop[i, :]
...
def parallel(runs, popsize, gmax, vt, voltage, current, points):
    ...
    for i in prange(runs):
        ...
        score, solution = differential_evolution(popsize, gmax, vt, voltage, current, points)
        score_list[i] = score
        solution_list[i, :] = solution
    return score_list, solution_list

# --- Main Loop ------ #
def process_iv_curve(results_folder, iv_folder, iv_file, vmin, vmax, runs, popsize, gmax, temperature_celsius):
    ...
    voltage, current = load_raw_data(iv_folder, iv_file, vmin, vmax, output_folder=None, write_to_file=False)
    
    points = np.size(voltage)
    score_list = np.zeros((runs, 1))
    solution_list = np.zeros((runs, 7))
    score_list, solution_list = parallel(runs, popsize, gmax, vt, voltage, current, points)
...
def main(iv_folder, results_folder, vmin, vmax, runs, popsize, gmax, temperature_celsius):
    ...
    for i in range(total_files):
        ...
        process_iv_curve(results_folder, iv_folder, iv_curves[i], vmin, vmax, runs, popsize, gmax, temperature_celsius)
        sys.stdout.flush()